...
test_detach(void)
{
	...
	unit_check(thread_task_detach(t) == TPOOL_ERR_TASK_NOT_PUSHED,
		   "can't detach a not pushed task");
	unit_check(thread_pool_push_task(p, t) == 0, "pushed");
	unit_check(thread_task_delete(t) == TPOOL_ERR_TASK_IN_POOL,
		   "can't delete before join/detach");
	unit_check(thread_task_detach(t) == 0, "detached");

	usleep(500000); // >:(

	unit_check(thread_pool_delete(p) == 0, "pool delete");
